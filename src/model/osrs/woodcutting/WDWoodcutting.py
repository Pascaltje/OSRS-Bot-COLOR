...
class OSRSWDWoodcutting(OSRSBot):
    ...
    def create_options(self):
        ...
        self.options_builder.add_dropdown_option("log_type", "What type of logs?",
        ...
        self.options_builder.add_slider_option("delay_min", "How long to take between actions (min) (MiliSeconds)?",
                                               300, 3000)
        self.options_builder.add_slider_option("delay_max", "How long to take between actions (max) (MiliSeconds)?",
                                               650, 3000)
    ...
    def setup(self):
        ...
        self.deposit_ids = [ids.BIRD_NEST, ids.BIRD_NEST_5071, ids.BIRD_NEST_5072, ids.BIRD_NEST_5073,
                            ids.BIRD_NEST_5074, ids.BIRD_NEST_5075, ids.BIRD_NEST_7413, ids.BIRD_NEST_13653,
                            ids.BIRD_NEST_22798, ids.BIRD_NEST_22800, self.log_type]
    ...
    def pick_up_nests(self):
        ...
        while self.pick_up_loot(
                ["Bird nest", "Clue nest (easy)", "Clue nest (medium)", "Clue nest (hard)", "Clue nest (elite)"]):
    ...
    def adjust_camera(self, color, timeout=4):
        ...
        start_time = time.time()  # let's make sure we don't wait too long
    ...
    def find_bank(self, deposit_slots):
        ...
        while not self.mouseover_text(contains="Bank"):
            ...
            if search_tries > 5:
                self.log_msg(
                    f"Did not see 'Bank' in mouseover text after {search_tries} searches, quitting bot so you can fix it...")
                self.stop()
            search_tries += 1
        ...
        while not self.api_m.get_is_player_idle():
            ...
            if time.time() - wait_time > 15:
                self.log_msg(
                    "We clicked on the bank but player is not idle after 10 seconds, something is wrong, quitting bot.")
                self.stop()
            time.sleep(self.random_sleep_length(.8, 1.3))
    ...
    def is_woodcutting(self):
        ...
        woodcutting_animation_list = [animation.WOODCUTTING_3A_AXE, animation.WOODCUTTING_BRONZE,
                                      animation.WOODCUTTING_IRON, animation.WOODCUTTING_STEEL,
                                      animation.WOODCUTTING_BLACK, animation.WOODCUTTING_MITHRIL,
                                      animation.WOODCUTTING_ADAMANT, animation.WOODCUTTING_RUNE,
                                      animation.WOODCUTTING_DRAGON]
    ...
    def roll_break(self, percentage, minutes_since_last_break, seconds, deposit_slots):
        if (
                rd.random_chance(probability=percentage / (
                1 if self.afk_train else 4))  # when afk there's weird timing issues, so we divide by 4 if not afk
    ...
    def reset_timer(self, minutes_since_last_break, seconds, percentage, deposit_slots):
        self.log_msg(
            f"Break time, last break was {minutes_since_last_break} minutes and {seconds} seconds ago. \n Chance of random break was {round(percentage * 100)}%")
    ...
    def go_afk(self, percentage):
        ...
        self.log_msg("Player is woodcuter, lets go afk...")
        if self.is_focused:
            self.switch_window()
        self.breaks_skipped = 0
        chopping_time = 0
